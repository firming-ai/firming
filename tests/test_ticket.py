...
import firming
from firming import Status, Ticket, job
...
def test_submit_returns_immediately_with_a_pending_ticket():
    ...
    ticket = firming.submit(jobs, "8h", venues=[venue])
...
def test_collect_without_waiting_returns_none_until_the_batch_lands():
    ...
    assert firming.collect(ticket, venues=[venue], wait=False) is None
    assert firming.collect(ticket, venues=[venue], wait=False) is None
    results = firming.collect(ticket, venues=[venue], wait=False)
    assert results is not None and results[0].ok and results[0].text == "batch:x"
    assert not ticket.pending
    assert firming.receipt(results).captured_pct == pytest.approx(50.0)


def test_status_peeks_without_collecting():
    venue = ProviderSide()
    ticket = firming.submit([job("claude-haiku-4-5", "x")], "8h", venues=[venue])
    states = firming.status(ticket, venues=[venue])
...
def test_a_ticket_round_trips_through_json_and_resumes_in_a_fresh_process(tmp_path):
    ...
    ticket = firming.submit(jobs, "8h", venues=[first])
    ...
    second = ProviderSide()
    results = firming.collect(later, venues=[second], poll_interval=0)
...
def test_partial_collection_survives_a_restart(tmp_path):
    ...
    ticket = firming.submit(jobs, "8h", venues=[fast, slow])
    assert firming.collect(ticket, venues=[fast, slow], wait=False) is None
    ...
    results = firming.collect(later, venues=[ProviderSide(), Other()], poll_interval=0)
...
def test_collect_past_the_buffer_settles_through_the_fallback():
    ...
    ticket = firming.submit(jobs, "2h", venues=[venue], risk_buffer=10**9)

    results = firming.collect(ticket, venues=[venue], wait=False)  # buffer already reached
...
def test_a_venue_missing_at_collect_time_fails_its_jobs_clearly():
    venue = ProviderSide(polls_to_complete=10_000)
    ticket = firming.submit([job("claude-haiku-4-5", "x")], "2h", venues=[venue], risk_buffer=10**9)
    results = firming.collect(ticket, venues=[], wait=False)
...
def test_an_empty_submit_is_an_empty_settled_ticket():
    ticket = firming.submit([], "8h", venues=[ProviderSide()])
    assert not ticket.pending and firming.collect(ticket, venues=[]) == []
